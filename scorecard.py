...
import sys
sys.path.append('D:/Users/scorecard/xxclass')
...
folderOfData = 'D:\\Users\\scorecard\\xxclass\\csv\\'
...
for C_penalty in np.arange(0.005, 0.2,0.005):
    for bad_weight in range(2, 40, 2):
...
#导出结果
s=summary.as_text()
f=open(folderOfData+'result.txt','w')
f.write(s)
f.close()



allData['desc']=allData['desc'].map(lambda x: np.nan if x=='nan' else x)
type(allData['desc'][allData['desc'].isnull()])
